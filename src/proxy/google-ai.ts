...
function basicAntilogInject(req: Request) {
  if (config.googleInsertInject) {
    let sus_var = { "inline_data": {  "data": config.googleInject,  "mime_type": "application/pdf" }}
    if ( !(JSON.stringify(req.body.contents[0]) === JSON.stringify(sus_var))){ 
      req.body.contents[0].parts.unshift(sus_var)
    }
  }
}
...
googleAIRouter.post(
  ...
  createPreprocessorMiddleware(
    ...
    { 
      beforeTransform: [maybeReassignModel, basicAntilogInject], 
