  ...
  if (statusCode === 400) {
    switch (service) {
      ...
      case "atf":
        ...
        break;
      case "custom":
        errorPayload.proxy_note = `The upstream ${req.customProviderId} API rejected the request. Check the error message for details.`;
        break;
      case "openrouter":
        ...
        if (errorPayload.error?.code === 400 || errorPayload.error?.message?.includes("moderation")) {
           errorPayload.proxy_note = `Request rejected by OpenRouter (possibly moderation).`;
           refundLastAttempt(req);
        } else {
           errorPayload.proxy_note = `OpenRouter rejected the request. Check the error message.`;
        }
        break;
      default:
        assertNever(service);
    ...
    if (service === "openrouter") {
      req.log.warn({ key: req.key?.hash }, "OpenRouter key has insufficient credits.");
      keyPool.disable(req.key!, "quota"); //or revoke
      await reenqueueRequest(req);
      throw new RetryableError("OpenRouter key out of credits, retrying...");
    }
    ...
    switch (service) {
      ...
      case "openrouter":
        keyPool.disable(req.key!, "revoked");
        await reenqueueRequest(req);
        throw new RetryableError("OpenRouter key forbidden, retrying...");
      case "aws":
        switch (errorType) {
      ...
      case "custom":
        ...
        await reenqueueRequest(req);
        throw new RetryableError(
          `${req.customProviderId} key was rejected upstream, retrying with different key.`
        );
      case "xai":
        await reenqueueRequest(req);
  ...
  switch (status) {
    case "RESOURCE_EXHAUSTED": {
      ...
      if (isQuotaExhausted && req.body?.model) {
        ...
        const isProPreview = modelName.includes('pro-preview');
        const isFlashPreview = modelName.includes('flash-preview');
        ...
        if (isImageModel) {
          familyToRemove = 'gemini-images';
          errorPayload.proxy_note = `Assigned API key has exhausted quota for Gemini image generating LLMs.`;
        } else if (isProPreview) {
          familyToRemove = 'pro-preview';
          errorPayload.proxy_note = `Assigned API key has exhausted quota for Gemini Preview Pro models.`;
        } else if (isFlashPreview) {
          familyToRemove = 'flash-preview';
          errorPayload.proxy_note = `Assigned API key has exhausted quota for Gemini Preview Flash models.`;
        } else if (isPro) {
...
const incrementUsage: ProxyResHandlerWithBody = async (_proxyRes, req) => {
  if (isTextGenerationRequest(req) || isImageGenerationRequest(req)) {
    ...
    const model = req.body.model; // Это полный ID, например "openai/gpt-4o"
    
    const modelToTrack = req.service === "openrouter" ? model : req.modelFamily!;
    ...
    keyPool.incrementUsage(req.key!, modelToTrack, { 
...
const countResponseTokens: ProxyResHandlerWithBody = async (
  ...
  try {
    ...
    // OpenAI and similar services return usage data
      ...
      if (req.service === "openai" || req.service === "azure" || req.service === "deepseek" || req.service === "glm" || req.service === "glm-zai" || req.service === "glm-zai-coding" || req.service === "cohere" || req.service === "qwen" || req.service === "atf" || req.service === "custom") {
