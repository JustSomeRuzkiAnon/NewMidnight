...
type Config = {
  ...
  glmKey?: string;
  /**
   * Comma-delimited list of GLM-ZAI (z.ai) API keys.
   */
  glmZaiKey?: string;
  /**
  ...
  maxContextTokensGoogleAI: number;
  ...
  maxOutputTokensGoogleAI: number;
  ...
  /** Which model families to block requests for. Applies only to OpenAI. */
  blockedModelFamilies: ModelFamily[];
  /**
  ...
  googleInsertInject?: boolean;
  googleInject?: string;
  allowModeratedOpenRouter?: boolean;
...
export const config: Config = {
  ...
  maxContextTokensGoogleAI: getEnvWithDefault("MAX_CONTEXT_TOKENS_GOOGLEAI", 307200),
  ...
  maxOutputTokensGoogleAI: getEnvWithDefault(
    ["MAX_OUTPUT_TOKENS_GOOGLEAI", "MAX_OUTPUT_TOKENS"],
    8192
  ),
  ...
  blockedModelFamilies: parseCsv(
    getEnvWithDefault("BLOCKED_MODEL_FAMILIES", "")
  ) as ModelFamily[],
  ...
  googleInsertInject: getEnvWithDefault("GOOGLE_INSERTINJECT", false),
  googleInject: getEnvWithDefault("GOOGLE_PDF_INJECT", generateUnixTimePseudoQrPdf()),
  allowModeratedOpenRouter: getEnvWithDefault("OPENROUTER_ALLOW_MODERATED", false),
  allowPaidOpenRouter: getEnvWithDefault("OPENROUTER_ALLOW_NON_FREE", true),
} as const;
