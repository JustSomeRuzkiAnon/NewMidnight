...
const OPENROUTER_SUB_FAMILIES: ModelFamily[] = [
  "OpRout_OpenAI",
  "OpRout_Anthropic",
  "OpRout_Google_AI_Studio",
  "OpRout_XAI",
...
export function buildInfo(baseUrl: string, forAdmin = false): ServiceInfo {
  ...
  const hasOpenRouterKey = keys.some(k => k.service === "openrouter" && !k.isDisabled);
  ...
  if (hasOpenRouterKey) {
    OPENROUTER_SUB_FAMILIES.forEach(f => accessibleFamilies.add(f));
  }
...
function addKeyToAggregates(k: KeyPoolKey) {
  ...
  switch (k.service) {
    ...
    case "openrouter":
      ...
      OPENROUTER_SUB_FAMILIES.forEach((fam) => {
        addToFamily(`${fam}__active`, !k.isDisabled ? 1 : 0);
        addToFamily(`${fam}__revoked`, k.isRevoked ? 1 : 0);
        
        const orKey = k as any;
        if (orKey.tier) {
          addToFamily(`${fam}__trial`, orKey.tier === 'restricted' ? 1 : 0);
          addToFamily(`${fam}__overQuota`, orKey.tier === 'deadkey' ? 1 : 0);
        }

        const usage = k.tokenUsage?.[fam];
        if (usage) {
          const input = usage.input || 0;
          const output = usage.output || 0;
          
          addToFamily(`${fam}__inputTokens`, input);
          addToFamily(`${fam}__outputTokens`, output);
          
          // Это починит "tookens" в шапке
          sumInputTokens += input;
          sumOutputTokens += output;
          
          sumCost += getTokenCostUsd(fam, input, output);
        }
      });
      break;
    ...
    case "aws": {
      ...
      if (!k.isDisabled) {
        ...
        k.modelIds.forEach((id) => {
          ...
          } else if (id.includes("claude-4-5-opus")) {
    ...
    case "google-ai":
	  ...
	  if (family === 'gemini-pro' || family === 'gemini-flash' || family === 'gemini-ultra' || family === 'pro-preview' || family === 'flash-preview' || family === 'gemini-images' ) {
    ...
    case "glm":
      ...
      break;
    case "glm-zai":
      if (!keyIsGlmZaiKey(k)) throw new Error("Invalid key type");
      k.modelFamilies.forEach((f) => {
        incrementGenericFamilyStats(f);
        addToFamily(`${f}__overQuota`, k.isOverQuota ? 1 : 0);
      });
