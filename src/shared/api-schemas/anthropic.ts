...
export const transformAnthropicChatToOpenAI: APIFormatTransformer<
  typeof OpenAIV1ChatCompletionSchema
> = async (req) => {
  const { body } = req;
  
  const result = AnthropicV1MessagesSchema.safeParse(body);
  if (!result.success) {
    req.log.warn(
      { issues: result.error.issues, body },
      "Invalid Anthropic-to-OpenAI request"
    );
    throw result.error;
  }

  const anthropicData = result.data;
  const messages: any[] = [];

  // 1. System Prompt
  if (anthropicData.system) {
    let systemText = "";
    if (typeof anthropicData.system === "string") {
      systemText = anthropicData.system;
    } else if (Array.isArray(anthropicData.system)) {
      systemText = anthropicData.system
  ...
  // 2. Messages
  for (const msg of anthropicData.messages) {
    const role = msg.role;
    ...
    for (const block of content) {
      // --- TEXT ---
      if (block.type === "text") {
        // TS знает, что тут есть text, так как это общее свойство или проверено
        if ('text' in block) {
      ...
      // --- IMAGE ---
        ...
        // Кастуем к any, чтобы достать source
    ...
    if (openAIContent.length > 0 || toolCalls.length > 0) {
      const message: any = {
        ...
        content: openAIContent.length === 1 && openAIContent[0].type === "text" 
          ...
          : (openAIContent.length > 0 ? openAIContent : null)
      };
  ...
  if (anthropicData.tools && anthropicData.tools.length > 0) {
    tools = anthropicData.tools.map((tool: any) => ({
      type: "function",
      function: {
        name: tool.name,
  ...
  if (anthropicData.tool_choice && tools) {
    const tc = anthropicData.tool_choice as any;
    if (tc.type === "auto") {
      tool_choice = "auto";
    } else if (tc.type === "any") {
  ...
  const thinking = (anthropicData as any).thinking;
  if (thinking) {
  ...
  return {
    ...
    stream: anthropicData.stream || false,
    max_tokens: anthropicData.max_tokens,
    // OpenAI схема требует number, а у Anthropic number | undefined. Ставим дефолты.
    temperature: anthropicData.temperature ?? 1,
    top_p: anthropicData.top_p ?? 1,
    top_k: anthropicData.top_k,
    stop: anthropicData.stop_sequences,
    tools: tools,
