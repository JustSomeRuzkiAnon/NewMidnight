import { AtfKey } from "./provider";
import { logger } from "../../../logger";
import { assertNever } from "../../utils";
import { getAtfUrl } from "../../atf";
...
export class AtfKeyChecker {
  private log = logger.child({ module: "key-checker", service: "atf" });

  constructor(
    private readonly update: (hash: string, key: Partial<AtfKey>) => void
  ) {}

  public async checkKey(key: AtfKey): Promise<void> {
    try {
      const result = await this.validateKey(key);

      // If we get here, reset any connection error counters since the request
      // succeeded.
      ...
      if (result === "server_error") {
        ...
        if (currentCount <= MAX_SERVER_ERROR_RETRIES) {
          ...
          setTimeout(() => {
            this.log.info(
              { hash: key.hash },
              "Retrying key check after server error"
            );
            this.checkKey(key);
      ...
      if (currentCount <= MAX_CONNECTION_ERROR_RETRIES) {
        ...
        setTimeout(() => {
          this.log.info(
            { hash: key.hash },
            "Retrying key check after connection error"
          );
          this.checkKey(key);
  ...
  /**
   * ATF's upstream is another proxy, so we only check that the key gets past
   * its gatekeeper. A model listing is used rather than a completion so that
   * checking keys doesn't consume the upstream's quota.
   */
      ...
      switch (response.status) {
        ...
        case 403:
          this.log.warn(
            ...
            "Key was rejected by the upstream proxy"
          );
          return "invalid";
        case 402:
