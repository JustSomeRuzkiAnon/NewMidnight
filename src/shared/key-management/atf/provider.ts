...
import { AtfKeyChecker } from "./checker";
import { config } from "../../../config";
import { logger } from "../../../logger";
import { AtfModelFamily } from "../../models";

export interface AtfKey extends Key {
  readonly service: "atf";
  readonly modelFamilies: AtfModelFamily[];
  isOverQuota: boolean;
}

export class AtfKeyProvider implements KeyProvider<AtfKey> {
  readonly service = "atf";

  private keys: AtfKey[] = [];
  private checker?: AtfKeyChecker;
  private log = logger.child({ module: "key-provider", service: this.service });

  constructor() {
    const keyConfig = config.atfKey?.trim();
    if (!keyConfig) {
      return;
    }
  ...
  public init() {
    ...
    this.checker = new AtfKeyChecker(this.update.bind(this));
    for (const key of this.keys) {
      void this.checker.checkKey(key);
    }
  }

  public get(_model: string): AtfKey {
    const availableKeys = this.keys.filter((k) => !k.isDisabled);
    if (availableKeys.length === 0) {
      throw new Error("No ATF keys available");
    }
  ...
  public list(): Omit<AtfKey, "key">[] {
    return this.keys.map(({ key, ...rest }) => rest);
  }

  public disable(key: AtfKey): void {
  ...
  public update(hash: string, update: Partial<AtfKey>): void {
  ...
  public incrementUsage(
    ...
    // ATF only has one model family "atf"
  ...
  public markRateLimited(keyHash: string) {
    ...
    key.rateLimitedUntil = now + AtfKeyProvider.RATE_LIMIT_LOCKOUT;
  }
  ...
  private throttle(hash: string) {
    ...
    const nextRateLimit = now + AtfKeyProvider.KEY_REUSE_DELAY;
