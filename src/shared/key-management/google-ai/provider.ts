...
import { estimateGoogleAITokenCount } from '../../tokenization/openai';
import { GoogleAIChatMessage } from "../../api-schemas/google-ai";
...
/**
 * Determines if request passing free RPM based on context size.
 * Returns TRUE if the request fits within limits (is "small").
 * Returns FALSE if the request is huge (needs paid key).
 * Returns TRUE (allow pass) if body/contents are missing to avoid blocking errors.
 */
function isPassingFreeRPM(model: string, request: any): boolean {
  //const req = getCurrentRequest();
  // 1. БЕЗОПАСНАЯ ПРОВЕРКА: Если body нет или нет contents, считаем что всё ок (или обрабатываем как хотим)
  if (!request || !request.body.contents) {
    return true; 
  }

  const prompt: GoogleAIChatMessage[] = request.body.contents;
  
  try {
    const result = estimateGoogleAITokenCount(prompt).token_count;

    console.log(`Current model: ${model}. Pre-RPM check tokens: ${result}`);  
      
    if (model.includes("2.0-flash")){
      return result < 990000;
    }
    
    if (model.includes("flash")){
      return result < 246500;
    }
    
    // Для Pro и остальных
    return result < 123500;

  } catch (error) {
    console.warn("Failed to count tokens for Google AI check, assuming passing:", error);
    return true;
  }
}
...
export class GoogleAIKeyProvider implements KeyProvider<GoogleAIKey> {
  ...
  public get(model: string, streaming?: boolean, requestBody?: any) {
    ...
    if (isPreviewModel(model)) {
      ...
      if (!isPassingFreeRPM(model, requestBody)) {
        availableKeys = availableKeys.filter((k) => k.billingEnabled === true);
        if (availableKeys.length === 0) {
          throw new PaymentRequiredError(
            "No billing-enabled Google AI keys available for high-context requests"
          );
        }
      }
