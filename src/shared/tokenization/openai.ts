    ...
    for (const key of Object.keys(message)) {
      {
        ...
        if (textContent.length > 7200000 || numTokens > 800000) {
...
export function getGpt4VisionTokenCost(imageUrl: string): number {
  // 1. Проверка на data URL
  if (imageUrl.startsWith("data:")) {
    const matches = imageUrl.match(/^data:([^;]+);base64,(.+)$/);
    
    // Если формат кривой, но это data-url, считаем как текст (fallback)
    if (!matches) {
      return Math.ceil(imageUrl.length / 4); 
    }

    const mimeType = matches[1];
    const base64Data = matches[2];

    // 2. Если это PDF или текст (не картинка)
    if (!mimeType.startsWith("image/")) {
      // Для PDF и текстовых файлов Gemini/OpenRouter считают токены от содержимого.
      // Мы не можем распарсить PDF здесь, поэтому берем грубую оценку:
      // 1 байт ~= 0.25-0.5 токена. Base64 увеличивает размер на 33%.
      // Грубая эвристика: длина base64 / 4.
      return Math.ceil(base64Data.length / 4);
    }

    // 3. Если это картинка — пытаемся посчитать по логике OpenAI Vision (тайлы)
    try {
      const buffer = Buffer.from(base64Data, "base64");
      // Нам нужны размеры. В Node.js без внешних либ (типа sharp) это сложно.
      // Обычно в таких проксях используется библиотека 'image-size'
      ...
      // Если не удалось декодировать картинку (или нет либы image-size),
      // возвращаем дефолтную стоимость (например, как за 1 картинку высокого разрешения)
      // 85 (base) + 170 * tiles. Пусть будет ~1000 токенов для безопасности.
      return 1000; 
    }
  }

  // 4. Если это ссылка (http...), кидаем ошибку, так как прокся не умеет качать файлы
  throw new Error(
    "Remote images are not supported. Add the image to your prompt as a base64 data URL."
  );
}
...
/**
 * Стандартная формула OpenAI для подсчета токенов картинок
 * (Low res = 85, High res = скейлинг + тайлы по 170 токенов)
 */
function calculateImageTokens(width: number, height: number, detail: "low" | "high" | "auto" = "auto"): number {
  if (detail === "low") return 85;

  // Скейлинг под 2048x2048
  if (width > 2048 || height > 2048) {
  ...
  // Скейлинг так, чтобы короткая сторона была 768px
